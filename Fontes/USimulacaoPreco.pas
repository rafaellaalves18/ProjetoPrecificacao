...
type
  TFrmSimulacaoPreco = class(TForm)
    ...
    procedure txtCodigo_ProdutoExit(Sender: TObject);
    procedure btnCancelarClick(Sender: TObject);
    procedure qrysimulacaoPERC_FRETEChange(Sender: TField);
    procedure qrysimulacaoPERC_ICMSChange(Sender: TField);
    procedure qrysimulacaoPERC_OUTRAS_DESPESASChange(Sender: TField);
    procedure qrysimulacaoPERC_IPIChange(Sender: TField);
    procedure qrysimulacaoPERC_PISChange(Sender: TField);
    procedure qrysimulacaoPERC_COFINSChange(Sender: TField);
  private
...
procedure TFrmSimulacaoPreco.btnCancelarClick(Sender: TObject);
begin
   Formshow(nil);
end;
...
begin
   If Application.MessageBox('Confirma Exclusão da Simulação?','Atenção',MB_ICONQUESTION + MB_YESNO + MB_DEFBUTTON2) = idNo Then
     Exit;
...
procedure TFrmSimulacaoPreco.qrysimulacaoPERC_COFINSChange(Sender: TField);
begin
   CalculaValorSugerido();
end;

procedure TFrmSimulacaoPreco.qrysimulacaoPERC_FRETEChange(Sender: TField);
begin
   CalculaValorSugerido();
end;

procedure TFrmSimulacaoPreco.qrysimulacaoPERC_ICMSChange(Sender: TField);
begin
   CalculaValorSugerido();
end;

procedure TFrmSimulacaoPreco.qrysimulacaoPERC_IPIChange(Sender: TField);
begin
   CalculaValorSugerido();
end;

procedure TFrmSimulacaoPreco.qrysimulacaoPERC_OUTRAS_DESPESASChange(
  Sender: TField);
begin
   CalculaValorSugerido();
end;

procedure TFrmSimulacaoPreco.qrysimulacaoPERC_PISChange(Sender: TField);
begin
   CalculaValorSugerido();
end;
...
begin
   ...
   FrmConsultaProduto.iTag := 1;
   FrmConsultaProduto.ShowModal;
   FrmConsultaProduto.Free;

end;

procedure TFrmSimulacaoPreco.txtCodigo_ProdutoExit(Sender: TObject);
begin
    if txtCodigo_Produto.Text <> '' then
    begin
        qryaux.Close;
        qryaux.SQL.Text := ' select descricao_prod from produto where codigo_produto ='+txtCodigo_Produto.Text;
        qryaux.Open();
        if qryaux.RecordCount > 0 then
           txtDescricao_Produto.Text := qryaux.FieldByName('descricao_prod').AsString;
